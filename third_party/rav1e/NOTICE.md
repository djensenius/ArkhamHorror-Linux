...
- **Name:** rav1e
- **Upstream:** https://github.com/xiph/rav1e
- **License:** BSD-2-Clause (see [`LICENSE`](./LICENSE), reproduced verbatim
  from upstream)
- **Copyright:** 2017-2023, the rav1e contributors

## Why this may be bundled

rav1e is one of the AV1 codec backends the system `libavif` package
...
scans the populated AppDir for `librav1e.so*` immediately after
`linuxdeploy`'s automatic `ldd`-based dependency bundling and copies this
notice into the distributed AppImage at
`usr/share/doc/ArkhamHorror/third_party/rav1e/` only when that library is
...
given AppImage build if rav1e is genuinely part of that build's bundled
codec closure.

## License scope

This NOTICE and the accompanying `LICENSE` file document rav1e's own

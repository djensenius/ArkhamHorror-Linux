# Third-party dependency: systemd (libsystemd)
...
- **Name:** systemd (libsystemd)
- **Upstream:** https://github.com/systemd/systemd
- **License:** LGPL-2.1-or-later (see `LICENSE`, the canonical FSF LGPLv2.1
  text)
- **Copyright:** Copyright (C) The systemd contributors

## Why this may be bundled

libsystemd is a transitive dependency of bundled D-Bus/glib (journal and
logind integration) on systemd-based distributions; linuxdeploy's ldd-based
bundling copies it automatically when present in that closure.
...
distributed AppImage's `usr/share/doc/ArkhamHorror/third_party/systemd/`
only when this library is actually found bundled in a given build.

## License scope

This NOTICE and the accompanying license file(s) document systemd's own

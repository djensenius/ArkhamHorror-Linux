# Third-party dependency: libgav1
...
- **License:** Apache License, Version 2.0 (see [`LICENSE`](./LICENSE),
  reproduced verbatim from upstream)
- **Copyright:** Google LLC (per upstream source file headers)

## Why this may be bundled

libgav1 is one of the AV1 codec backends the system `libavif` package
...
scans the populated AppDir for `libgav1.so*` immediately after
`linuxdeploy`'s automatic `ldd`-based dependency bundling and copies this
notice into the distributed AppImage at
`usr/share/doc/ArkhamHorror/third_party/libgav1/` only when that library is
...
given AppImage build if libgav1 is genuinely part of that build's bundled
codec closure.

## License scope

This NOTICE and the accompanying `LICENSE` file document libgav1's own
